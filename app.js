...
const DEFAULT_WEBHOOK = "https://script.google.com/macros/s/AKfycbwc14hS0vVAtzwVzmsu7xOzTCmpfY59GzS96cNfp5ATLNxH8jrX_JCAoXUsaTbBwqg6ag/exec";
...
async function init(){
  const local=lsLoad();
  const url=localStorage.getItem(WH_KEY)||DEFAULT_WEBHOOK;
  ...
  if(url){
    ...
    if(data&&data.sessions!==undefined){
      ...
      if(url===DEFAULT_WEBHOOK) localStorage.setItem(WH_KEY,url);
      loadSession();
      render();
      return;
    }
